...
function videosSlide() {
    ...
    const videos = {},
        zIndexList = {},
        initialSize = {};
    [...document.getElementsByClassName('modelVideos')].forEach((el, i) => { videos[i] = el;
        initialSize[el] = el.getBoundingClientRect().width });
    ...
    function spin(x) {
        ...
        for (const key in videos) {
            ...
            el.style.left = Math.sin(totalSwippedId / boundries.x) * limit * (isOffScreen ? .8 : 1.2) + 'px';
...
function Textboxes() {
    ...
    document.getElementById('infoArrowRight').addEventListener('click', infoToRight);
    document.getElementById('infoArrowLeft').addEventListener('click', infoToLeft);
